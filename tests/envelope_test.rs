...
async fn envelope_clamps_autonomously_on_limit_tightening_with_no_command() -> Result<()> {
    ...
    // Arrange — seed rack cache entries + generous envelope limits so the
    // envelope task has everything it needs and isn't binding yet.
    ...
    // Act — a real operator command: 200kW, equal_split -> 100kW/rack.
    operator
        .publish(Message::new(
    ...
    operator
        .publish(Message::new(
            format!("sites/{SITE_ID}/devices/{MODULE_ID}/measurements/active_power/watts"),
            r#"{"ts":"t","value":100000.0}"#,
            0,
        ))
        .await?;

    // Assert — within a couple of 1 Hz ticks, both racks hold the clamped,
    // equal-split share of 50_000: 25_000 each. Poll instead of a fixed
    // sleep since the envelope task's tick boundary isn't observable
    // directly.
    let clamped = timeout(Duration::from_secs(10), async {
        loop {
            let r1 = read_rack_watts(rack1_port).await?;
            let r2 = read_rack_watts(rack2_port).await?;
            if r1 == 25_000 && r2 == 25_000 {
                return anyhow::Ok(());
            }
    ...
    assert!(
        clamped.is_ok(),
        "envelope task never autonomously clamped both racks to 25_000"
    );
